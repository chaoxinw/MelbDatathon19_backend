...
# context.use_privatekey_file('/etc/letsencrypt/live/sugeromelbdata.com/privkey.pem')
# context.use_certificate_file('/etc/letsencrypt/live/sugeromelbdata.com/fullchain.pem')
	...
	def post(self):
		...
		if inputGeo_go.intersects( result ):
			...
			if len(needed_ij_list) == 0:
				return { "errorCode" : 1  }
			if len(needed_ij_list) > 200:
				return { "errorCode" : 1, "errorMsg": "Area too big"  }
...
if __name__ == '__main__':
	context = ('/etc/letsencrypt/live/sugeromelbdata.com/fullchain.pem', '/etc/letsencrypt/live/sugeromelbdata.com/privkey.pem')
